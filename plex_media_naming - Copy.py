...
yearRx = '([\(\[\.\-])([1-2][0-9]{3})([\.\-\)\]_,+])'  #
translation_table = maketrans("`", "'")  # ("`e??â??", "':/")
...
def roman_to_int(string):
    roman_values = [['X', 10], ['IX', 9], ['V', 5], ['IV', 4],
                    ['I', 1]]  # ['M',1000],['CM',900],['D',500],['CD',400],['C',100],['XC',90],['L',50],['XL',40]
    result = 0
    string = string.upper()
...
def Log(entry, filename='Plex Media Scanner Custom.log'):  # need relative path):
    ...
    try:
        ...
        with open(filename,
                  'a') as file:  # with open("/volume1/Plex/Library/Application Support/Plex Media Server/Logs/" + filename, 'a') as file:
...
def clean_filename(string):
    ...
    string = re.sub(r'\(.*?\)', '',
                    string)  # remove "(xxx)" or "[xxx]" groups as Plex cleanup keep inside () but not inside []
    string = re.sub(r'\[.*?\]', '',
                    string)  # remove "(xxx)" or "[xxx]" groups as Plex cleanup keep inside () but not inside []
    string = re.sub(r'\{.*?\}', '',
                    string)  # remove "(xxx)" or "[xxx]" groups as Plex cleanup keep inside () but not inside []
    ...
    for group in release_groups:  string = string.replace(group,
                                                          " ");  # Remove known tags not in '()' or '[]', generally surrounded by '_'
    words = string.split(" ")
    for word in words:
        if word != "":
            for term in whack_strings:
                if word.lower() == term.lower():
                    try:
                        ...
                        Log("word: '%s', words: '%s'" % (word, str(words)))
            for rx in whackRx:
                if re.sub(rx, "", word) == "" and word in words:
                    try:
                        words.remove(word)
                    except:
                        Log("word: '%s', words: '%s', rx: '%s'" % (word, str(words), rx))
...
def add_episode_into_plex(mediaList, files, file, show, season=1, episode=1, episode_title="", year=None,
                          endEpisode=None):
